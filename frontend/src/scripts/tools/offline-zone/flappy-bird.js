...
const FlappyBird = {
    ...
    // Configurações do pássaro (otimizadas para jogabilidade)
    bird: {
        x: 80,
        y: 250,
        width: 34,
        height: 24,
        velocity: 0,
        gravity: 0.45,  // REDUZIDO de 0.5 para 0.45 (cai mais devagar)
        jumpStrength: -8.5,  // AJUSTADO de -9 para -8.5 (pulo mais controlável)
        rotation: 0,
        maxRotation: 90,
        minRotation: -25
    },
    
    // Configurações dos canos (balanceadas para ser POSSÍVEL)
    pipes: [],
    pipeWidth: 70,
    pipeGap: 200,  // AUMENTADO de 160 para 200 (mais fácil de passar)
    pipeSpeed: 2,  // REDUZIDO de 2.5 para 2 (mais tempo para reagir)
    pipeFrequency: 110,  // AUMENTADO de 100 para 110 (mais espaço entre canos)
    frameCount: 0,
    ...
    init() {
        ...
        console.log('✅ Flappy Bird inicializado!', {
            ...
            birdPosition: `(${this.bird.x}, ${this.bird.y})`
        });
    },
    ...
    startGame() {
        ...
        this.particles = [];
        this.frameCount = 0;
        this.pipeSpeed = 2;  // AJUSTADO para 2 (início mais fácil)
        this.updateScore();
        ...
        if (startBtn) startBtn.innerHTML = '🔄 Reiniciar';
    ...
    gameLoop(currentTime = 0) {
        ...
        const deltaTime = Math.min((currentTime - this.lastTime) / 16.67, 2); // Limitar delta
        this.lastTime = currentTime;
        
        this.update(deltaTime);
        this.draw();
        
        // Continuar loop apenas se ainda estiver jogando
    ...
    update(deltaTime = 1) {
        ...
        for (let i = this.pipes.length - 1; i >= 0; i--) {
            ...
            if (!pipe.scored && pipe.x + this.pipeWidth < this.bird.x) {
                ...
                if (this.score % 10 === 0 && this.pipeSpeed < 4) {  // Máximo 4 (antes era 4.5)
                    this.pipeSpeed += 0.25;  // Aumenta 0.25 a cada 10 pontos
                }
            }
    ...
    spawnPipe() {
        const minGap = 80;  // AUMENTADO de 50 para 80 (limites mais generosos)
        const maxGap = this.canvas.height - 140;  // AJUSTADO de 120 para 140
    ...
    checkCollision(pipe) {
        ...
        const hitbox = {
            x: this.bird.x + 6,  // AUMENTADO de 4 para 6 (hitbox menor)
            y: this.bird.y + 4,  // AUMENTADO de 2 para 4
            w: this.bird.width - 12,  // AUMENTADO de 8 para 12
            h: this.bird.height - 8   // AUMENTADO de 4 para 8
        };
    ...
    endGame() {
        ...
        const savedHighScore = (typeof Utils !== 'undefined' ? Utils.loadData('flappy_bird_highscore') : null) || 0;
