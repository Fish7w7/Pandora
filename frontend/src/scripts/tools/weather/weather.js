const Weather = {
    get apiKey() { return Utils.loadData('openweather_api_key') || ''; },
    set apiKey(value) { if (value) Utils.saveData('openweather_api_key', value); },
        ...
        if (this.apiKey) {
            return `
                <div class="bg-green-50 border-2 border-green-300 rounded-xl p-3 mb-3 flex items-center justify-between">
                    ...
                    <button onclick="Weather.clearApiKey()" class="text-xs text-red-600 hover:text-red-800 font-semibold px-3 py-1 border border-red-300 rounded-lg hover:bg-red-50 transition-all">
                        🗑️ Remover Key
                    </button>
                </div>
            `;
        ...
        return `
            <div class="bg-yellow-50 border-2 border-yellow-300 rounded-xl p-4 mb-3">
                ...
                <div class="flex gap-2">
                    ...
                    <button onclick="Weather.saveApiKey()" class="px-5 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-xl font-bold text-sm transition-all">
                        Salvar
                    </button>
                </div>
            </div>
    ...
    saveApiKey() {
        ...
        if (!key) { Utils.showNotification('❌ Digite uma API Key válida', 'error'); return; }
        this.apiKey = key;
            ...
            <div class="bg-white rounded-2xl shadow-2xl p-4 mb-3">
                <div class="mb-3">
                    <label class="block text-gray-700 font-bold mb-2 text-sm flex items-center gap-2">
                        <span>🏙️</span><span>Buscar Cidade</span>
                    </label>
                    <div class="flex gap-2">
                        ...
                        <button onclick="Weather.searchCity()"
                            ...
                            <span class="text-lg">🔍</span><span>Buscar</span>
                        </button>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2 mb-3">
                    <span class="text-gray-600 font-semibold flex items-center">Exemplos:</span>
                    ${[['São Paulo','🇧🇷','blue'],['Rio de Janeiro','🇧🇷','green'],['London','🇬🇧','purple'],['Tokyo','🇯🇵','red']].map(([city, flag, color]) =>
                        `<button onclick="document.getElementById('city-input').value='${city}'; Weather.searchCity()"
                                class="px-4 py-2 bg-${color}-100 hover:bg-${color}-200 text-${color}-800 rounded-lg font-semibold text-sm transition-all">
                            ${flag} ${city}
                        </button>`
                    ).join('')}
                </div>
                <div class="text-center">
                    <button onclick="Weather.getLocationWeather()"
                        ...
                        <span class="text-lg">📍</span><span>Usar Minha Localização</span>
                    </button>
                </div>
            </div>
        `;
    ...
    renderResult() { return `<div id="weather-result" class="hidden"></div>`; },
    ...
    async searchCity() {
        ...
        if (!city) { Utils.showNotification('❌ Digite o nome de uma cidade', 'error'); return; }
        if (!this.apiKey) { Utils.showNotification('⚠️ Configure sua API Key primeiro!', 'warning'); return; }
        this.showLoading();
        try {
    ...
    async fetchWeatherData(city) {
        ...
        if (!currentResponse.ok) throw new Error(await this.parseApiError(currentResponse));
        ...
        if (forecastResponse.ok) this.forecast = await forecastResponse.json();
    },

    async parseApiError(response) {
        const errorData = await response.json().catch(() => ({}));
        const errors = { 401: 'API Key inválida ou não ativada', 404: 'Cidade não encontrada. Tente outro nome', 429: 'Limite de requisições excedido. Aguarde alguns minutos' };
        return errors[response.status] || `Erro ${response.status}: ${errorData.message || 'Erro desconhecido'}`;
    },

    async getLocationWeather() {
        if (!this.apiKey) { Utils.showNotification('⚠️ Configure sua API Key primeiro!', 'warning'); return; }
        if (!navigator.geolocation) { Utils.showNotification('❌ Geolocalização não suportada', 'error'); return; }
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            error => {
                this.hideLoading();
                const errors = { 1: 'Permissão negada', 2: 'Localização indisponível', 3: 'Tempo esgotado' };
            ...
            <div class="animate-fadeIn">
                ...
                <div class="mt-3 bg-green-50 border-2 border-green-300 rounded-xl p-3">
                    <div class="flex items-center gap-2 text-green-800">
                        ...
                        <div><div class="font-bold text-sm">Dados carregados com sucesso!</div><div class="text-xs">API Key funcionando perfeitamente</div></div>
                    </div>
                </div>
            </div>
        `;
            ...
            <div class="bg-gradient-to-br ${gradient} rounded-3xl shadow-2xl overflow-hidden">
                <div class="p-4 text-white text-center">
                    <h2 class="text-2xl font-black mb-2 flex items-center justify-center gap-2">
                        <span>📍</span><span>${data.name}, ${data.sys.country}</span>
                    </h2>
                    ...
                    <div class="flex items-center justify-center gap-3 text-sm">
                        <span>🔽 Min: ${tempMin}°C</span><span>•</span><span>🔼 Máx: ${tempMax}°C</span>
                    </div>
                </div>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-2 p-4 bg-white/10 backdrop-blur-sm">
                    ${[['💧','Umidade',`${data.main.humidity}%`,''],['💨','Vento',Math.round(data.wind.speed*3.6),'km/h'],['👁️','Visibilidade',(data.visibility/1000).toFixed(1),'km'],['🌡️','Pressão',data.main.pressure,'hPa']].map(([icon, label, value, unit]) =>
                        `<div class="bg-white/20 backdrop-blur-sm rounded-xl p-3 text-center text-white transform hover:scale-105 transition-all">
                            ...
                            ${unit ? `<div class="text-xs opacity-80">${unit}</div>` : ''}
                        </div>`
                    ).join('')}
                </div>
        ...
        return `
            <div class="bg-white rounded-2xl shadow-2xl p-4 mt-3">
                <h3 class="text-lg font-black text-gray-800 mb-3 flex items-center gap-2">
                    <span>📅</span><span>Previsão para os Próximos Dias</span>
                </h3>
                <div class="grid grid-cols-2 md:grid-cols-5 gap-2">
                    ${dailyForecast.map(day => {
                        ...
                        <div class="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-lg p-2 text-center hover:shadow-lg transition-all transform hover:scale-105">
                            ...
                            <img src="${this.urls.icon}/${day.weather[0].icon}@2x.png" alt="${day.weather[0].description}" class="w-10 h-10 mx-auto">
                            <div class="text-lg font-black text-gray-800">${Math.round(day.main.temp)}°C</div>
                            <div class="text-xs text-gray-600 capitalize">${day.weather[0].description}</div>
                        </div>`;
                    }).join('')}
                </div>
            </div>
    ...
    hideLoading() { document.getElementById('weather-loading')?.classList.add('hidden'); }
};
